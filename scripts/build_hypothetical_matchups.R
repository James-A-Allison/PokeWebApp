...
missing_hypo_matchups <- dbReadTable(con, "missing_hypo_matchups")
dbDisconnect(con, shutdown = FALSE)

missing_hypo_matchups <- missing_hypo_matchups %>%
  inner_join(upcoming_bosses %>%
  select(raid_boss = `Raid Boss`, raid_tier = Tier)) %>%
  filter(raid_tier == 5) %>%
  inner_join(get_base_stats() %>%
  select(pokemon_name = name, lv_40_cp = `Lv 40 CP`) %>%
  distinct()) %>%
  arrange(desc(lv_40_cp))

pokemon_to_loop <- missing_hypo_matchups %>% select(pokemon_name) %>% distinct() %>% pull()

rm(missing_hypo_matchups)

for (i in seq(pokemon_to_loop)) {
  loop_mon <- pokemon_to_loop[i]
